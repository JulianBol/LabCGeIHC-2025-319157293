...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		spotLights[1].SetFlash(glm::vec3( 32.5 + mainWindow.getmuevex(), 3.5f, 6.3f), glm::vec3(1.0, -0.5, 0.0)); // Faro Auto
		...
		// Para prender y apagar luz puntual (Lampara)
		if (mainWindow.getcntrlLuzLamp()) {
			shaderList[0].SetPointLights(pointLights, pointLightCount);
		}
		else shaderList[0].SetPointLights(pointLights, pointLightCount - 1); 
