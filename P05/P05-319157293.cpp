...
int main()
{
	...
	Car = Model();
	Car.LoadModel("Models/P05-car.obj");

	Car_Chasis = Model();
	Car_Chasis.LoadModel("Models/P05-car_chasis.obj");

	Car_Faros = Model();
	Car_Faros.LoadModel("Models/P05-car_faros.obj");

	Car_Tire = Model();
	Car_Tire.LoadModel("Models/P05-car_tire.obj");

	Car_Trunk = Model();
	Car_Trunk.LoadModel("Models/P05-car_trunk.obj");
	...
	while (!mainWindow.getShouldClose())
	{
		...
		// CARRO POR PARTES PARA DARLE UN COLOR DISTINTO A CADA PARTE (CHASIS, FAROS Y CUERPO DE CARRO)
		...
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion7()), glm::vec3(0.0, 0.0, 1.0));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion6()), glm::vec3(0.0, 0.0, 1.0));


		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));

		color = glm::vec3(0.603f, 0.713f, 0.764f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
