...
public:
	...
	void CreateMeshGeometryMulticolor(std::vector<GLfloat>& vertices, std::vector<unsigned int>& indices, unsigned int numOfVertices, unsigned int numberOfIndices);
	void RenderMesh();
	void RenderMeshGeometry();
	void RenderMeshGeometryAristas();
	void ClearMesh();
	~Mesh();
private:
	GLuint VAO, VBO, IBO;
